...
- Recompute steps: `29`
- M14 read-only script steps: `28`
- Acceptance gates: `9`
- Two-pass stabilization required: `True`
- Rescue no-ledger count: `0`
...
- Pre-refresh review audit rows/ready/waiting/backfill: `17/0/17/0`
- Future source-reextract spec prep rows/drafts/unblocked/blocked/pending/legacy-history inputs: `2/2/0/2/16/0`
...
Post-fresh-refresh recompute checklist has 29 steps, including 28 read-only M14 script steps and 9 acceptance gates. Current evidence still waits for fresh refresh: fresh_refresh_observed=True, quote_source=longbridge_quote_readonly, post-refresh waiting rows=0. The checklist requires two-pass objective/decision stabilization and keeps final-discard allowed at 0. Pre-refresh review audit has 17 rows, with 0 supporting-artifact backfills. Future source-reextract spec prep has 2 rows, 0 unblocked rows, 16 pending confirmations, and 0 legacy-history planning inputs. Strategy next-step matrix currently has 20 rows and 0 legacy-history planning inputs. Internal-sim trial acceptance has 1/7 start-ready rows, 6 fresh-refresh-required rows, and 0 legacy-history planning inputs. Manual M12.37 once-mode, broker/live, real orders, paper approval, parameter mutation, registry/account-spec mutation, and broker readiness mutation remain disabled.

## Preconditions

- `m12_47_supervisor_owned_refresh`: `observed` - fresh_refresh_observed=True; quote_source=longbridge_quote_readonly.
- `approved_runtime_inputs_connected`: `ready` - approved runtimes connected 7/7.
...
1. `wait_for_m12_47_supervisor_refresh` (supervisor_refresh)
   ...
   - Current state: `ready_after_refresh`
...
2. `review_post_refresh_outcomes` (evidence_recompute)
   ...
   - Current state: `ready_after_refresh`
   - Acceptance hint: waiting rows should fall from current 0 after fresh evidence exists.
3. `refresh_rescue_ab_evidence` (evidence_recompute)
   - Command: `python scripts/run_m14_rescue_ab_evidence_tracker.py`
   - Current state: `ready_after_refresh`
   - Acceptance hint: no-ledger rescue rows should be rechecked from current 0.
...
22. `strategy_source_visual_confirmation_response_gate_refresh` (decision_stabilization)
   - Command: `python scripts/run_m14_strategy_source_visual_confirmation_response_gate.py`
   - Current state: `ready_after_refresh`
   - Acceptance hint: manual visual responses should be re-read; pending question and case counts must remain explicit.
23. `strategy_future_source_reextract_spec_prep_refresh` (decision_stabilization)
   - Command: `python scripts/run_m14_strategy_future_source_reextract_spec_prep.py`
   - Current state: `ready_after_refresh`
   - Acceptance hint: conditional future source-reextract spec prep should stay blocked until manual visual confirmation, with legacy-history planning inputs at 0.
24. `objective_blocker_burndown_refresh` (decision_stabilization)
...
25. `strategy_next_step_readiness_matrix_refresh` (decision_stabilization)
...
26. `internal_sim_trial_acceptance_gate_refresh` (decision_stabilization)
...
27. `strategy_pre_refresh_review_packet_refresh` (decision_stabilization)
...
28. `strategy_pre_refresh_review_audit_refresh` (decision_stabilization)
...
29. `project_stage_assessment_refresh` (final_assessment)
...
- `fresh_refresh_source_gate`: `passed` - Current fresh_refresh_observed=True, source_quote=longbridge_quote_readonly.
- `approved_internal_sim_runtime_gate`: `passed` - 7/7 approved runtime inputs connected.
- `rescue_first_ledger_gate`: `passed` - Current no-ledger rescue count=0.
- `parameter_shadow_review_gate`: `passed` - Current shadow-review candidates=3; mutation allowed=0.
...
- `internal_sim_trial_acceptance_gate`: `waiting` - trial ready/approved=1/7; fresh-required=6; legacy inputs=0.
