...
Zero-signal rescue diagnostics reviewed 9 rescue runtimes. 7 are blocked mainly by stale/non-fresh quote source and should be rechecked on the next M12.47 fresh refresh; 1 need parameter/filter work; 1 have no parent source rows for the configured timeframe. Potential entries if fresh quote gate clears: 63. No broker connection, real order, live execution, or paper-trading approval is enabled.
...
- Dominant issue: `parent_source_absent_for_timeframe`
- Parent source rows: `0`
- Eligible if fresh quote: `0`
- Rejection reasons: `{}`
- Quote sources: `{}`
- Sample symbols: `none`
- Action: Fix parent detector/timeframe source mapping before waiting for more A/B days.
...
- Action: Test a shadow-only reward/R normalization family, such as 1.0R or 1.1R minimum, before changing the frozen rescue runtime.
...
- Dominant issues: `{'parent_source_absent_for_timeframe': 1, 'reward_filter_blocks_all': 1, 'stale_quote_source_blocks_candidate': 7}`
