...
- 已新增 M14 post-fresh-refresh recompute checklist：`scripts/run_m14_post_fresh_refresh_recompute_checklist.py` 会把下一次 M12.47 fresh refresh 后的重算顺序、验收门和边界固化成 `m14_post_fresh_refresh_recompute_checklist.json/md`；当前清单含 `24` 步、`23` 个只读 M14 script steps、`7` 个 acceptance gates，并要求 objective audit / objective execution / strategy decision ladder / strategy evidence gap matrix / strategy evidence gap burndown / strategy pre-refresh review packet / strategy pre-refresh review audit 稳定化后再刷新 project stage assessment。当前仍 `fresh_refresh_observed=false`、`quote_source=fallback_quotes_only`、`13` 条 post-refresh watch rows 等待；该 checklist 不触发 M12.37 once-mode、不接 broker、不改参数、不改 registry/account specs/broker readiness，并已接入 project stage assessment。
...
- 已新增 M14 strategy future source-reextract spec prep：`scripts/run_m14_strategy_future_source_reextract_spec_prep.py` 会只读 source reextract review 与 visual confirmation response gate，把 `M10-PA-003`、`M10-PA-010` 整理为 `2` 条条件式 future spec 草案准备行，包含 `10` 个 source-backed atoms、`6` 个 source-review answers、`2` 个 conditional drafts；当前人工视觉确认未完成，`0` 条 unblocked、`2` 条 blocked_until_manual_visual_confirmation、`16` 个待确认项。该 prep 明确写入 `legacy_historical_profit_planning_input=false`，账户下钻旧版历史净利润/历史收益只允许 display-only，不得进入策略规划、创建、晋级、淘汰、参数激活或 broker readiness；已接入 objective completion audit 和 project stage assessment。
- 已新增 M14 objective blocker burndown：`scripts/run_m14_objective_blocker_burndown.py` 会只读 objective audit、project stage、evidence burndown 与 source visual response gate，生成 `m14_objective_blocker_burndown.json/md`；当前 `7` 条 blocker rows，`P0/P1/P2=4/3/0`，其中明确把账户下钻里的旧版 `historical_net_profit` / 历史收益类字段列为污染风险并排除在策略 promotion、rescue priority、参数 activation、broker readiness 和 objective completion 决策之外，`legacy_historical_profit_planning_input_count=0`。当前仍是 `10/10` challenge、`3` 个 approved internal-sim 策略可继续内部模拟，但 `2` 个 rescue runtime 等首个 M13 ledger、`10` 条 rescue A/B gap、`6` 个视觉问题和 `10` 个 case 回复仍未完成；不允许 broker/live、真实订单、paper approval、参数 mutation 或手动 M12.37 once-mode。
...
- 已新增 M14 strategy next-step readiness matrix：`scripts/run_m14_strategy_next_step_readiness_matrix.py` 会只读 strategy decision ladder、evidence gap matrix/burndown、objective blocker、source visual response gate、rescue A/B evidence 和 parameter shadow spec，生成 `m14_strategy_next_step_readiness_matrix.json/md`；当前 `20` 条策略路线中，`3` 条只能继续下一次 M12.47-supervised 内部模拟刷新，`10` 条进入 rescue/影子参数复核，`7` 条保持 source/plugin research review-only。该矩阵把账户下钻旧版历史净利润/历史收益继续固定为 display-only，`legacy_historical_profit_planning_input_count=0`，并保持 promotion、final discard、parameter activation、broker paper start 全部为 `0`；不允许 broker/live、真实订单、paper approval、参数 mutation、registry/account-spec/broker readiness mutation 或手动 M12.37 once-mode。
- 已把 M14 strategy next-step readiness matrix 与 internal-sim trial acceptance gate 接入 post-fresh recompute checklist 和 project stage assessment：`m14_post_fresh_refresh_recompute_checklist.json/md` 现在把 objective blocker burndown、next-step matrix 与 trial acceptance gate 纳入 M12.47 fresh refresh 后重算顺序，清单为 `27` 步、`26` 个只读 M14 script steps、`9` 个 acceptance gates，包含 legacy-history exclusion gate 与 internal-sim trial acceptance gate；`m14_project_stage_assessment.json/md` 现在直接汇总 next-step matrix 的 `20` 条策略桶位、`3/10/7` 路线分布，以及 trial gate 的 `3/3` start-ready、`3` 条 fresh-refresh-required、global gates `pass=3 / waiting=2`、`legacy_historical_profit_planning_input_count=0`，顶层阶段判断不再只依赖孤立报告。
...
- 已新增 M14 internal simulated-account trial acceptance gate：`scripts/run_m14_internal_sim_trial_acceptance_gate.py` 会只读 next-session plan、next-step matrix、project stage assessment、post-fresh checklist、objective blocker 和 post-refresh outcome，生成 `m14_internal_sim_trial_acceptance_gate.json/md`；当前 `3/3` 个 approved 策略可进入下一次 M12.47-supervised 内部模拟试运行，`3/3` 都仍需 fresh refresh 后验收，global gates 为 `pass=3 / waiting=2`，`M10-PA-005 / M10-PA-008` 只能带 broker dry-run blocker watch 继续，`legacy_historical_profit_planning_input_count=0`、broker paper/live/manual M12.37/参数 mutation 全部继续禁用。
