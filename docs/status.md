...
- 已把 M14 strategy next-step readiness matrix 接入 post-fresh recompute checklist 和 project stage assessment：`m14_post_fresh_refresh_recompute_checklist.json/md` 现在把 objective blocker burndown 与 next-step matrix 纳入 M12.47 fresh refresh 后重算顺序，清单从 `24` 步扩为 `26` 步、只读 M14 script steps 从 `23` 增至 `25`、acceptance gates 从 `7` 增至 `8`，新增 legacy-history exclusion gate；`m14_project_stage_assessment.json/md` 现在直接汇总 next-step matrix 的 `20` 条策略桶位、`3/10/7` 路线分布和 `legacy_historical_profit_planning_input_count=0`，顶层阶段判断不再只依赖孤立报告。
