...
- 当前 M12.17 实现无代码侧阻塞；每日看板已可连续落盘，但模拟买卖试运行复查还差 `9` 个交易日记录。
...
- 下一步进入 M12.18：把 `M10-PA-008/009` 转成严格定义观察，不混入 `M10-PA-001/002/012/FTD` 每日主线，也不直接进入模拟买卖准入。
