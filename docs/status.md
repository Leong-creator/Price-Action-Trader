...
- 已新增 M14 rescue optimization backlog：`scripts/run_m14_rescue_optimization_backlog.py` 会把 rescue plan、rescue A/B evidence 与 M14.2 broker dry-run blocker 合成提前优化队列；当前显示 `9/9` 条 rescue 变体在 10 日 A/B 未满前已有可处理项，其中 `8` 条是已接线但零信号、`1` 条是有信号但未形成 account operation，另有 `3` 个 broker dry-run 风控阻断事件分布在 `M10-PA-005 / M10-PA-008`。该队列只用于优化/诊断，不计作 promotion，也不允许 broker/live。
