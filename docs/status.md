...
- 已把 M14 post-refresh outcome review 接入 project stage assessment：`scripts/run_m14_project_stage_assessment.py` 现在把 goal readiness、next-session plan、rescue plan、rescue backlog 与 `m14_rescue_post_refresh_outcome_review` 合成项目阶段和策略路线图；当前阶段为 `M14 stable strategy testing + M14.2 broker readiness dry-run scaffold`，10 日 challenge 已 `10/10`，`M10-PA-004 / M10-PA-005 / M10-PA-008` 继续内部模拟，救援侧为 `9/11` 条已有 M13 ledger、`2` 条等首个 ledger、promotion allowed `0`。post-refresh 当前 `fresh_refresh_observed=false`、`quote_source=fallback_quotes_only`、`13/13` watch rows 仍等待 M12.47 fresh refresh、`passed_count=0`、`failed_count=0`；路线计数为 `3` 条继续内部模拟、`8` 条 rescue A/B、`1` 条 parallel A/B、`1` 条 rebuild detector、`7` 条 shadow/plugin/research；仍不允许手动 M12.37 once-mode、broker paper/live、真实订单或 paper approval。
...
- 已新增 M14 post-fresh-refresh recompute checklist：`scripts/run_m14_post_fresh_refresh_recompute_checklist.py` 会把下一次 M12.47 fresh refresh 后的重算顺序、验收门和边界固化成 `m14_post_fresh_refresh_recompute_checklist.json/md`；当前清单含 `21` 步、`20` 个只读 M14 script steps、`7` 个 acceptance gates，并要求 objective audit / objective execution / strategy decision ladder / strategy evidence gap matrix 稳定化后再刷新 project stage assessment。当前仍 `fresh_refresh_observed=false`、`quote_source=fallback_quotes_only`、`13` 条 post-refresh watch rows 等待；该 checklist 不触发 M12.37 once-mode、不接 broker、不改参数、不改 registry/account specs/broker readiness，并已接入 project stage assessment。
