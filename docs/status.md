...
- 已新增 M14 project stage assessment：`scripts/run_m14_project_stage_assessment.py` 把 goal readiness、next-session plan、rescue plan 与 rescue backlog 合成项目阶段和策略路线图；当前阶段为 `M14 stable strategy testing + M14.2 broker readiness dry-run scaffold`，10 日 challenge 已 `10/10`，`M10-PA-004 / M10-PA-005 / M10-PA-008` 继续内部模拟，救援侧为 `9/11` 条已有 M13 ledger、`2` 条等首个 ledger、promotion allowed `0`。路线计数为 `3` 条继续内部模拟、`8` 条 rescue A/B、`1` 条 parallel A/B、`1` 条 rebuild detector、`7` 条 shadow/plugin/research；仍不允许手动 M12.37 once-mode、broker paper/live、真实订单或 paper approval。
