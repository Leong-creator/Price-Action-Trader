...
- M12.47 当前已修复 Longbridge 超时导致看板不刷新的运行问题：`quote` / `kline history` 仍先尝试 Longbridge 只读接口；若连续超时，则切换到公开 Yahoo chart 只读行情兜底，并在 artifact 中标记 `yahoo_chart_readonly_after_longbridge_error`，不伪装成 Longbridge 成功、不接账户、不下单。北京时间 `2026-05-30 00:43:09` 样本已恢复 `first50_daily_ready_symbols=50`、`first50_current_5m_ready_symbols=50`、`current_day_runtime_ready=true`、`data_freshness_warning=''`；INTC 当日缓存价格已回到约 `118` 区间。
