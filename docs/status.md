...
- 已新增 M14 rescue zero-signal diagnostics：`scripts/run_m14_rescue_zero_signal_diagnostics.py` 会只读 M12 dashboard 与 rescue coverage，拆解零信号 runtime 的父策略输入和过滤失败原因；当前 `9` 个零信号 runtime 中，`7` 个主要是当前 recompute/audit 快照报价源非 `longbridge_quote_readonly`，应等 M12.47 下一次 fresh quote 刷新后复核；`M10-PA-012-m14-modify-20260522-5m` 是 `reward_r_below_min` 阻断，适合做 shadow-only `1.0R/1.1R` 目标归一化测试；`M10-PA-013-m14-modify-20260522-1d` 缺父策略 1d 输入，需修 timeframe/source mapping。该诊断已接入 M14 goal readiness，不改变 broker/live 边界。
