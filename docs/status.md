...
- 已把旧版历史净利润隔离下沉到 M14 internal simulated-account next-session plan：`scripts/run_m14_internal_sim_next_session_plan.py` 现在把 `legacy_historical_profit_planning_input=false` 写入硬边界、策略会话行、acceptance checks 和 P0 planning boundary watch，配置若打开该字段会直接失败；最新 `m14_internal_sim_next_session_plan.json/md` 显示 `legacy_historical_profit_planning_input_count=0`、`global_watch_row_count=5`，因此下一次内部模拟会话计划也不会把账户下钻旧历史净利润用于策略规划、晋级、救援排序、参数激活或 broker readiness。
...
- 当前 M14 已把“是否真的测试过”升级为 challenge/gate 口径；有效 `10` 个纽约真实交易日 challenge 已固化为 `10/10`，但目标仍未完成。当前 blocker 是 rescue promotion 仍为 `0`、`2` 条 rescue runtime 还缺首个 M13 ledger、`10` 条 rescue A/B 证据未满、`11` 条 shadow-review 证据未完成，且 `13/13` 条 post-refresh watch rows 仍等待 M12.47-owned fresh refresh。任何单日 open/close、zero_signal 或 M13 goal complete 都不能直接解释成可投入使用或可盈利。
- M14 当前新增需要重点处理的策略 blocker：`M10-PA-001/002/007/009/012/013/M12-FTD-001` 等 rescue 变体要继续累计各自 10 日 A/B ledger；`M10-PA-011` 先重建 detector 再判断；plugin/filter/research 策略保持 shadow 覆盖，不伪装成独立交易账户。`M10-PA-004 / M10-PA-005 / M10-PA-008` 已可继续内部模拟，但 broker/live/real order/paper approval 仍全禁用。
...
- `M10-PA-004` 整体混合版本未达主线准入，但做多分支已转正并进入观察看板；`M10-PA-007` 可进入每日观察，但不得直接进入模拟买卖准入。
