...
- 已修复长桥实时链路提交口径和看板盈亏口径：长桥面板不再使用“长桥今日盈亏 / 长桥总盈亏”这类模糊字段，统一拆成“长桥账户当日盈亏”“长桥接口持仓今日浮动”“长桥当前持仓总盈亏”“长桥交易累计盈亏”；`Longbridge CLI` 返回空 JSON 或无订单号时会立刻反查长桥订单列表，能匹配到同标的、同方向、同数量、同价格订单就确认提交，找不到才记为未确认请求。`TEAM / TSLA` 这类可卖数量为 `0` 的重复平仓请求已被阻断，平仓信号 ID 也改为稳定指纹，避免每轮重复生成卖单。
- 已固定长桥模拟账户盈亏主口径：M12.32 长桥面板优先读取长桥只读 `profit-analysis / portfolio / positions / executions / orders` 对账结果；“长桥账户当日盈亏”优先用同日 `profit-analysis` 的 `sum_profit`，“长桥接口持仓今日浮动”只用 `portfolio.total_today_pl`，“长桥当前持仓总盈亏”只用当前持仓成本价/数量/当前价计算，“长桥交易累计盈亏”只用美股交易盈亏汇总。长桥接口缺数据时显示“无法计算/等待长桥数据”，禁止用 `100000`、旧本地模拟账本或旧 `historical_net_profit` 硬算。看板新增逐标的盈亏和逐策略成交盈亏归因；同一标的多策略参与时写成分摊估算。
...
- M12.47 当前已修复 Longbridge 超时导致看板不刷新的运行问题：`quote` / `kline history` 仍先尝试 Longbridge 只读接口；Longbridge CLI 调用现按官网建议默认固定 `LONGBRIDGE_REGION=cn` 与 `.cn` endpoint，避免自动选区落到不稳定 global endpoint。若 Longbridge 仍连续超时，则切换到公开 Yahoo chart 只读行情兜底，并在 artifact 中标记 `yahoo_chart_readonly_after_longbridge_error`，不伪装成 Longbridge 成功、不接账户、不下单。北京时间 `2026-05-30 14:34` 自检显示守护器存活但处于 `非交易日等待`；本地函数级探针已从 Longbridge 取回 `INTC.US` quote 与 2026-05-29 日 K，二者 close/last 均为 `114.680`。
...
- M15 当前已修复长桥看板盈亏口径混乱问题：M12 顶部不再显示模糊的“长桥总盈亏 / 长桥今日盈亏 / 长桥交易总盈亏”旧字段，统一改成“长桥账户当日盈亏”“长桥接口持仓今日浮动”“长桥当前持仓总盈亏”和“长桥交易累计盈亏”。其中 `portfolio.total_pl` 只代表当前持仓总浮盈浮亏，`portfolio.total_today_pl` 只代表当前持仓今日浮动，不能再叫账户总盈亏或当日总盈亏；账户当日盈亏优先使用同日 `profit-analysis` 的 `sum_profit`。本地分仓看板同步按相同口径展示主力仓、实验仓、002 专项主力仓的当日盈亏、持仓今日浮动、当前持仓总盈亏、已实现盈亏和交易累计盈亏；旧基线缺少资金池字段的成交仍显示为“未归属资金池”，只作历史归因说明，后续新订单按资金池写入。
