...
- 已接上 M14 rescue 最小输入流：`M10-PA-001-m14-modify-20260522`、`M10-PA-002-m14-modify-20260522`、`M10-PA-004-MBF-QC-m14-modify-20260522`、`M10-PA-007-m14-modify-20260522`、`M10-PA-009-m14-modify-20260522`、`M10-PA-012-m14-modify-20260522`、`M10-PA-013-m14-modify-20260522`、`M12-FTD-001-m14-modify-20260522`、`M10-PA-011-ORB-R1` 现在进入 M12 accountized runtime 的 `rescue` lane；其中 PA004-MBF-QC rescue 使用更严格的风险压缩 adapter，ORB-R1 使用 5m 失败开盘区间突破后回测确认 adapter，其余复用父策略扫描器并增加看涨、低风险、至少 `1.2R` 质量过滤；当前已登记 rescue runtime 均已接上输入流，后续不得伪装为已通过，只能继续用 10 日 A/B 数据判断。
- 已新增 M14 rescue runtime 覆盖审计：`scripts/run_m14_rescue_runtime_coverage.py` 会只读 M13 registry、M14 rescue plan 与 M12 account specs，生成 `m14_rescue_runtime_coverage.json/md`；当前登记 rescue runtime 为 `10/10` 策略、`11` 个账户全部接线，计划 rescue/rebuild 动作为 `10/10` 覆盖，但状态明确是 `connected_not_promoted`，仍需 10 个交易日 A/B ledger 才能 promote / modify / reject。
...
- 已新增 M14 rescue A/B evidence tracker：`scripts/run_m14_rescue_ab_evidence_tracker.py` 会只读 rescue coverage、M14 gate、M13 scorecard、M13 signal ledger 和 account ledger，生成 `m14_rescue_ab_evidence_tracker.json/md`；当前结果为 rescue runtime coverage 完整，既有 9 条 rescue 变体已有 M13 ledger 证据，新接入的 PA012 target/stop 影子 runtime 尚待下一次 M12.47/M13 刷新生成首条 ledger，因此当前为 `9/10` observed、`1` 条 no-ledger、promotion allowed 仍为 `0`。
- 已新增 M14 rescue optimization backlog：`scripts/run_m14_rescue_optimization_backlog.py` 会把 rescue plan、rescue A/B evidence 与 M14.2 broker dry-run blocker 合成提前优化队列；M13 已改为优先使用 M12 trade ledger 的显式 `trading_date`，旧 open 记录则回退到 `signal_time` 归属交易日，因此 `M10-PA-011-ORB-R1` 的 signal-to-account no-op 已清零并记录 `2` 条 open operation。当前显示 `10` 条 rescue 变体均有队列行，其中 `8` 条是已接线但零信号、`1` 条新 PA012 target/stop 影子 runtime 等待下一次 M12.47-owned fresh run 生成首条 M13 ledger、`0` 条是有信号但未形成 account operation，另有 `3` 个 broker dry-run 风控阻断事件分布在 `M10-PA-005 / M10-PA-008`。该队列只用于优化/诊断，不计作 promotion，也不允许 broker/live。
