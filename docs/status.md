...
- 已补强 M14 objective completion audit：`scripts/run_m14_objective_completion_audit.py` 会把当前用户目标拆成 `12` 项需求审计，并直接读取 parameter shadow spec、strategy decision ladder 与 strategy evidence gap matrix 作为弱策略救援/调参路径证据；当前 `proven=4`、`guardrail=3`、`in_progress=2`、`blocked=3`、`objective_complete=false`。已证明项目阶段、10 日 challenge、approved internal-sim 和外部参考映射；审计已显示 `14` 个参数影子变体、`10` 条 rescue-continuation ladder rows、`final_discard_allowed_count=0`，并把 strategy evidence gap matrix 的 `20` 条 open gap、`12` 条 fresh-refresh wait、`2` 条 first-ledger gap、`10` 条 rescue 10 日 A/B gap、`11` 条 shadow-review gap 纳入 blocker 说明；仍未完成的是 rescue promotion 证据、M12.47 fresh refresh 前置和整体目标完成。该审计已接入 project stage assessment，且继续禁止 broker/live、真实订单、paper approval、参数 mutation、registry/account-spec/broker readiness mutation 和手动 M12.37 once-mode。
