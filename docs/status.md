...
- M10 Price Action Strategy Refresh 已启动：
  ...
  - M12.38 已新增 Codex 观察员文件层：每次刷新会写入 `m12_38_codex_observer_latest.json` 和 `m12_38_codex_observer_inbox.jsonl`，内容是中文人话摘要，包含当前模拟权益、今日模拟盈亏、活跃周期、FTD001 风险和数据异常提醒；同时已创建 Codex App 当前线程心跳自动化 `m12`，每 `15` 分钟先运行一次 `python scripts/run_m12_37_intraday_auto_loop.py --once --no-fetch` 刷新只读看板，再读取产物并用中文汇报，只读不下单。
