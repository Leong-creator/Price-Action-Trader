...
- 已新增 M14 post-fresh-refresh recompute checklist：`scripts/run_m14_post_fresh_refresh_recompute_checklist.py` 会把下一次 M12.47 fresh refresh 后的重算顺序、验收门和边界固化成 `m14_post_fresh_refresh_recompute_checklist.json/md`；当前清单含 `20` 步、`19` 个只读 M14 script steps、`7` 个 acceptance gates，并要求 objective audit / objective execution / strategy decision ladder 做两轮稳定化后再刷新 project stage assessment。当前仍 `fresh_refresh_observed=false`、`quote_source=fallback_quotes_only`、`13` 条 post-refresh watch rows 等待；该 checklist 不触发 M12.37 once-mode、不接 broker、不改参数、不改 registry/account specs/broker readiness，并已接入 project stage assessment。
