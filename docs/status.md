...
- 已新增 M14 rescue A/B evidence tracker：`scripts/run_m14_rescue_ab_evidence_tracker.py` 会只读 rescue coverage、M14 gate、M13 scorecard、M13 signal ledger 和 account ledger，生成 `m14_rescue_ab_evidence_tracker.json/md`；当前结果为 rescue runtime coverage 完整但 M13 rescue ledger 证据 `0/9`，9 条 rescue 变体均为 `no_m13_rescue_ledger_evidence_yet`，父策略历史不得计入 rescue 变体 10 日 A/B 证据，promotion allowed 仍为 `0`。
