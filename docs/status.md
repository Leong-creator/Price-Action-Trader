...
- 已新增长桥模拟账户实时会话守护器：`scripts/run_m15_longbridge_realtime_session_supervisor.py` 和 `config/examples/m15_longbridge_realtime_session_supervisor.json` 只在美股常规交易时段按顺序运行实时行情采集、实时信号路由、实时账户状态、实时持仓退出和实时执行链路；非交易窗口只写等待状态。该守护器输出 `m15_longbridge_realtime_session_supervisor.json/jsonl/md`，明确记录 `legacy_fast_queue_used=false`、`manual_m12_37_once_used=false`、`local_ledger_input_ref=""`，并在连续失败达到阈值后熔断，避免异常时反复提交或反复拉起。
...
- 当前开盘值守状态：北京时间 `2026-06-04 20:26:49` / 纽约时间 `2026-06-04 08:26:49` 时，M12.47 守护器存活，M15 长桥实时守护器已用模拟订单启用版启动，长桥模拟账户只读状态已确认 `account_channel=lb_papertrading`、购买力约 `102044.44`、持仓 `2` 条、未完成挂单 `0` 条；开盘验收为 `armed_waiting_regular_session`，`9` 项通过、`1` 项等待交易窗口、`0` 项失败。到美股常规交易时段后，链路只会按 `10000 USD` 项目资金模型和实时风控提交长桥模拟账户订单，不会使用长桥账户大额余额放大仓位。
