...
- 已新增 M14 rescue runtime 覆盖审计：`scripts/run_m14_rescue_runtime_coverage.py` 会只读 M13 registry、M14 rescue plan 与 M12 account specs，生成 `m14_rescue_runtime_coverage.json/md`；当前登记 rescue runtime 为 `9/9` 策略、`10` 个账户全部接线，计划 rescue/rebuild 动作为 `10/10` 覆盖，但状态明确是 `connected_not_promoted`，仍需 10 个交易日 A/B ledger 才能 promote / modify / reject。
...
- 已新增 M14 rescue A/B evidence tracker：`scripts/run_m14_rescue_ab_evidence_tracker.py` 会只读 rescue coverage、M14 gate、M13 scorecard、M13 signal ledger 和 account ledger，生成 `m14_rescue_ab_evidence_tracker.json/md`；当前结果为 rescue runtime coverage 完整且 M13 rescue ledger 证据已进入 `9/9` 条 rescue 策略，9 条都处于 `collecting_ab_evidence`、当前各为 `1/10` 个 rescue A/B 交易日，父策略历史不得计入 rescue 变体 10 日 A/B 证据，promotion allowed 仍为 `0`。
- 已新增 M14 rescue optimization backlog：`scripts/run_m14_rescue_optimization_backlog.py` 会把 rescue plan、rescue A/B evidence 与 M14.2 broker dry-run blocker 合成提前优化队列；M13 已改为优先使用 M12 trade ledger 的显式 `trading_date`，旧 open 记录则回退到 `signal_time` 归属交易日，因此 `M10-PA-011-ORB-R1` 的 signal-to-account no-op 已清零并记录 `2` 条 open operation。当前显示 `9/9` 条 rescue 变体均有队列行，其中 `8` 条是已接线但零信号、`0` 条是有信号但未形成 account operation，另有 `3` 个 broker dry-run 风控阻断事件分布在 `M10-PA-005 / M10-PA-008`。该队列只用于优化/诊断，不计作 promotion，也不允许 broker/live。
...
- 已新增 M14 rescue target/stop shadow normalization：`scripts/run_m14_rescue_target_stop_shadow_normalization.py` 会只读 M12 dashboard 与 target/stop diagnostics，生成 PA012 影子目标价修复候选；当前 `risk_normalized_1_0r` 可让 `12/12` 条已过方向、杠杆、价格几何与风险门槛的候选达到 `1.0R`，中位目标价调整约 `0.0410%`，而 `opening_range_height_30m` 仍未突破原始低于 `1.0R` 的问题。该结果只是 shadow-only 设计输入，不登记新 runtime、不算 promotion、不允许 broker/live，后续仍需 fresh quote 后 10 个 rescue A/B 交易日证据。
