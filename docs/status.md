...
- 已接上 M14 rescue 最小输入流：`M10-PA-001-m14-modify-20260522`、`M10-PA-002-m14-modify-20260522`、`M10-PA-007-m14-modify-20260522`、`M10-PA-012-m14-modify-20260522` 现在进入 M12 accountized runtime 的 `rescue` lane，复用父策略扫描器但增加看涨、低风险、至少 `1.2R` 质量过滤；其余 rescue 仍保持 `not_connected`，不得伪装成已测试。
