...
- 已新增长桥模拟账户实时信号路由器：`scripts/run_m15_longbridge_realtime_signal_router.py` 和 `config/examples/m15_longbridge_realtime_signal_router.json` 只消费实时行情/新 K 线事件，输出 `m15_realtime_signal_events.jsonl` 给实时执行链路。该路由器明确写入 `local_simulation_isolated=true`、`local_ledger_input_ref=""`，不读取 `m12_46_account_trade_ledger.jsonl`、不读取旧快速队列、不使用本地平仓。修复策略、辅助模块和影子变体在信号生成阶段直接隔离，不再生成长桥待提交信号。第一版支持行情事件内嵌策略意图、`M10-PA-004-long-1d` 日线强势跟进检测器，以及 `price_action_realtime_v1` 通用实时价格行为检测器；后者已覆盖 `M10-PA-001-1d/5m`、`M10-PA-002-1d`、`M10-PA-005-1d/5m`、`M10-PA-008-1d`、`M10-PA-012-5m`、`M10-PA-013-1d/5m`、`M12-FTD-001-baseline-1d` 的白名单信号生成。同标的同方向多运行单元共振时只保留一条主信号，支持信号进入合并审计并提高主信号仓位上限，避免重复下单。
...
- 已新增长桥模拟账户实时持仓退出管理：`scripts/run_m15_longbridge_realtime_position_manager.py` 和 `config/examples/m15_longbridge_realtime_position_manager.json` 只看长桥账户持仓、长桥实时行情和长桥实时执行记录里的止损/目标价，触发后生成卖出已有多头的平仓信号；本地模拟平仓不会触发长桥平仓，卖出不会变成做空。长桥账户已有但没有本轮 M15 开仓元数据的持仓现在会标成非系统管理持仓，只展示、不自动平仓。
...
- 当前开盘值守状态：北京时间 `2026-06-05 10:50` 左右复核时，M12.47 守护器存活，M15 长桥实时守护器已用模拟订单启用版启动，当前不是美股常规交易时段，系统在等下一次常规交易窗口。长桥模拟账户只读状态已确认 `account_channel=lb_papertrading`、购买力/现金约 `100816.34`、持仓 `10` 条、未完成挂单 `0` 条；开盘验收为 `armed_waiting_regular_session`，`10` 项通过、`1` 项等待交易窗口、`0` 项失败，新增旧买入挂单清理检查也已通过。到美股常规交易时段后，链路只会按 `10000 USD` 项目资金模型和实时风控提交长桥模拟账户订单，不会使用长桥账户大额余额放大仓位。
