...
- M15 快速队列必须合并同一交易日、同一标的、同一方向的重复开仓候选：只保留一条主订单，其余作为共振支持行审计，不得重复提交。多策略共振只提高整股数量，倍率固定为基础 `1.0`、独立策略家族 `+0.50`、同家族变体 `+0.25`、最高 `1.75`，并继续受 `10000 USD` 资金模型、单标的 `1500 USD`、总敞口 `6000 USD`、单笔风险上限和扣费后预计净利润为正的硬门槛约束。标的价格不参与优先级，只参与整股、可买性和风控上限判断；候选提交顺序按扣费后预计净利润、策略质量、收益风险比、多策略共振排序。
