...
当前仓库已完成事件契约、SDK 客户端适配、订单号解析、增量事件传递和离线测试。SDK OAuth 已完成授权，并已用 SDK 账户余额与 CLI 的 `lb_papertrading` 现金、冻结现金和资产数据交叉核对。2026-07-14 已切换到 SDK 常驻行情和 SDK 模拟订单通道：启动器会先停止旧 CLI 实时守护器，防止两个来源同时写入同一事件文件；CLI 只保留每 60 秒账户、持仓和订单对账慢路径，不能阻塞 SDK 信号和下单热路径。147 只标的按每批 10 只订阅，避免单次大订阅超时。运行状态记录事件到执行热路径的最近、平均和 P95 耗时，真实订单请求耗时仍单独记录在执行流水中。
