...
- 实时事件文件在热路径只追加写入；每分钟心跳压缩为最近 20,000 条。SDK 运行进程保留最近 4,096 条必要 K 线上下文，但路由器只把本轮新事件作为信号触发源，执行器直接接收本轮新信号，不再从 JSONL 重扫下单输入。
...
当前仓库已完成事件契约、SDK 客户端适配、订单号解析、增量事件传递和离线测试。SDK OAuth 已完成授权，并已用 SDK 账户余额与 CLI 的 `lb_papertrading` 现金、冻结现金和资产数据交叉核对。SDK 行情只读订阅已连通，但尚未切换生产订单通道；切换前仍需让 SDK 与既有 CLI 采集器使用互斥事件文件并完成模拟订单集成验收。
