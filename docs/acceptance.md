...
- SDK 请求已发起但账户快照尚未出现订单时，对应资金池敞口不得释放。
- 订单已成交但精确成交归因尚未刷新时，敞口不得释放或重复计算。
- 精确归因确认开仓后，只由归因批次接管；确认已平仓或零成交终态后才释放。
- 日线结构连续成立时，后续行情事件只能复用同一个可提交实例；只有当前活跃行情的完整检测明确返回结构不成立后才可结束该实例，同一交易日后来形成的新结构可生成新实例。行情缺失、过期事件和未参与本轮检测不得重新放行；五分钟结构按闭合K线区分。
- 长桥实际持仓与虚拟归因必须逐标的数量一致；历史调整必须携带真实订单证据并排除策略成绩。
- 看板净资产数值、币种和来源必须来自同一口径；当日盈亏字段不可确认时不得显示本地估算值。
...
- M12.29 Current-day Scan + Minute Read-only Dashboard 必须满足：
  ...
  - 看板必须同时提供“策略成绩单”，按单策略独立展示当前账本表现、胜率、最大回撤、盈利因子、今日模拟盈亏和下一步状态；`M12-SRC-*` 来源回看候选不得作为独立策略重复出现在成绩单中。
...
- M12.34-M12.39 Intraday Observer Dashboard 必须满足：
  ...
  - `M12-FTD-001` 必须有独立重点监控卡，展示当前账本表现、胜率、最大回撤、今日触发股票、今日模拟盈亏、连续亏损参考和风险标记。
