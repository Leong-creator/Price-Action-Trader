        ...
        current.update(
            {
                ...
                "latest_broker_capacity_check": {
                    "checked_at": str(
                        latest_capacity.get("processed_at")
                        or latest_capacity.get("created_at")
                        or ""
                    ),
                    "status": str(latest_capacity.get("short_capacity_check_status") or ""),
                    "source": str(latest_capacity.get("short_capacity_source") or ""),
                    "maximum_quantity": latest_capacity.get("short_capacity_max_quantity"),
                    "cash_maximum_quantity": latest_capacity.get("short_capacity_cash_max_quantity"),
                    "margin_maximum_quantity": latest_capacity.get("short_capacity_margin_max_quantity"),
                    "basis": latest_capacity.get("short_capacity_basis"),
                    "blocker_class": latest_capacity.get("short_capacity_blocker_class"),
                    "cache_age_seconds": latest_capacity.get("short_capacity_cache_age_seconds"),
                    "query_ms": latest_capacity.get("short_capacity_query_ms"),
                }
                if latest_capacity
                else None,
                "broker_order_id_count": len(order_ids),
                "broker_any_filled_order_count": len(filled_order_ids),
    ...
    base_summary.update(
        {
            ...
            "broker_any_filled_order_count": sum(
                int(row.get("broker_any_filled_order_count") or 0)
                for row in runtime_rows.values()
            ),
        ...
        current.update(
            {
                ...
                "broker_order_id_count": len(order_ids),
                "broker_any_filled_order_count": len(filled),
                "broker_partially_filled_order_count": sum(
...
def build_dashboard(config: dict[str, Any], generated_at: str | None = None) -> dict[str, Any]:
    ...
    fill_summary = dict(fill_attribution.get("summary") or {})
    fill_fee_model = dict(fill_attribution.get("fee_model") or {})
    if "fees_are_estimated_not_actual" not in fill_summary:
        fill_summary["fees_are_estimated_not_actual"] = bool(
            fill_fee_model.get("fees_are_estimated_not_actual")
            or fill_summary.get("fee_source") == "configured_conservative_estimate"
            or fill_fee_model.get("source") == "configured_conservative_estimate"
        )
    fill_attribution["summary"] = fill_summary
    ...
    return {
        ...
        "runtime": {
            ...
            "sdk_push_subscription_coverage": runtime.get("trading_subscription_coverage"),
            "five_minute_session_coverage": runtime.get("five_minute_session_coverage") or {},
            "postclose_intraday_recovery": runtime.get("postclose_intraday_recovery") or {},
