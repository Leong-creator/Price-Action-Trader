...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a user-readable historical backtest demo on cached market data."
    )
    parser.add_argument(
        "--config",
...
def main() -> int:
    ...
    payload = {
        ...
        "split_summary_path": str(Path(outcome["report_dir"]) / "split_summary.json"),
        "regime_breakdown_path": str(Path(outcome["report_dir"]) / "regime_breakdown.json"),
        "knowledge_trace_coverage_path": str(Path(outcome["report_dir"]) / "knowledge_trace_coverage.json"),
        "no_trade_wait_path": str(Path(outcome["report_dir"]) / "no_trade_wait.jsonl"),
