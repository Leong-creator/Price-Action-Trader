...
自 `M9G.0` 起，旧 `PA-SC-*` strategy cards 与相关报告仅作为 legacy / historical baseline 保留；新一轮 Strategy Factory 从 `knowledge/indices/source_manifest.json` 重新开始，并使用独立的 `SF-*` 编号空间。
...
- 保留 `PA-SC-002` 与其他 `PA-SC-*` 产物作为历史对照与回归参考
- 在 `M9G.0` 之后，把新的 Strategy Factory 入口固定到 `config/strategy_factory/active_provider_config.json` 与 `reports/strategy_lab/strategy_factory/run_state.json`
