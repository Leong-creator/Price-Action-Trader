...
当前默认下一步不是继续 broker/live，也不是自动扩更多标的或更长窗口，而是进入 `M10.10 Visual Wave B Gate`，复核强图形策略是否能进入 Wave B 回测；继续停留在 `paper / simulated`。
