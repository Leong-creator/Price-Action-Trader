...
class LongbridgeDashboardTest(unittest.TestCase):
    ...
    def test_dashboard_uses_sdk_and_actual_longbridge_sources(self) -> None:
        ...
        runtime = _write(tmp_path / "runtime.json", {"runtime_engine": "sdk", "sdk_connected": True, "configured_symbol_count": 147, "subscribed_symbol_count": 147, "new_position_submission_enabled": True})
        account = _write(tmp_path / "account.json", {"paper_account_verified": True, "account_channel": "lb_papertrading", "position_row_count": 0, "open_order_count": 0})
        summary = _write(tmp_path / "summary.json", {"account_today_total_pnl": "12.3", "account_today_total_pnl_source": "longbridge"})
        ...
        orders = _write(tmp_path / "orders.json", {"summary": {"filled": 2}, "rows": []})
        pnl = _write(tmp_path / "pnl.json", {"account_pnl": {"sum_profit": "5"}, "source_status": {"ok": True}})
    ...
    def test_pending_flatten_forces_new_entries_off(self) -> None:
        ...
        files = {}
        for name, payload in {
            "runtime": {"runtime_engine": "sdk", "sdk_connected": True, "new_position_submission_enabled": True},
            "account": {"paper_account_verified": True, "position_row_count": 16},
