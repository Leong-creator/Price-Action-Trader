...
- M14 rescue target/stop shadow normalization 已落地：`scripts/run_m14_rescue_target_stop_shadow_normalization.py` 当前对 PA012 reward/R 阻断源行做 shadow-only 目标价生成候选，不登记新 runtime、不改变 frozen rescue runtime。最新结果显示 `risk_normalized_1_0r` 可让 `12/12` 条已过方向、杠杆、价格几何与风险门槛的 PA012 候选达到 `1.0R`，中位目标价调整约 `0.0410%`；`opening_range_height_30m` 仍无法突破原始低于 `1.0R` 的问题。下一步只能在 fresh quote 后把该候选作为 shadow A/B 设计输入，仍需 10 个 rescue A/B 交易日证据后才可 M14 review。
