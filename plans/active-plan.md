...
- 2026-08-06：今晚（纽约交易日 `2026-08-06`）使用独立编号 `m15-sdk-validation-20260806` / `m15-sdk-validation-short-20260806` 执行合同 v1 全链路自然信号验收；明晚（纽约交易日 `2026-08-07`）才启用正式统计编号 `m15-sdk-contract-v1-20260807` / `m15-sdk-contract-v1-short-20260807`，两组订单、成交和成绩不得混算。账户当前持仓、挂单和待确认订单均为 `0`；今晚常规交易时段开始后，平账户会自动开启验收，不制造订单、不降低门槛。纽约时间 `15:45` 的停止新开仓检查必须在每批行情进入路由前执行，再清理验收持仓；清理完成后继续等待次日正式编号。8 条可执行合同全部参与实时判断；`PA004-long / PA007 / PA008` 三条视觉合同只做只读影子验收，不生成订单。本次高风险切换须在今晚盘后人工复核真实订单号、成交、截止和清仓；失败时回退到 `pending_flatten` 并停止新开仓，不切回 CLI、不删除长桥审计数据。
...
- 2026-08-06（上一轮纽约交易日 `2026-08-05` 的验证记录）：上一轮自然信号验证结束后的 `20` 个长桥模拟持仓已由 SDK 市价清仓单全部成交，长桥事实源当前持仓 `0`、挂单 `0`。该记录不代表今晚纽约交易日 `2026-08-06` 的新验收已完成；今晚使用独立验收编号，明晚再启用正式编号。
