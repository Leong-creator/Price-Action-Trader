...
- 下一阶段已正式列入连续交付计划：`M12.12 Daily Observation Loop`，分支为 `feature/m12-12-daily-observation-loop`，目标是把 cache、scanner、只读观察和 dashboard snapshot 串成日常更新链路；缺少输入时必须 deferred，不得补假信号或假收益。
