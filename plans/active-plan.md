...
- M14 post-fresh-refresh recompute checklist 已落地：`scripts/run_m14_post_fresh_refresh_recompute_checklist.py` 当前把下一次 M12.47 fresh refresh 后的重算顺序、验收门和边界固化为 `23` 步清单，其中 `22` 个只读 M14 script steps、`7` 个 acceptance gates，并明确 objective audit / objective execution / strategy decision ladder / strategy evidence gap matrix / strategy evidence gap burndown / strategy pre-refresh review packet 需要稳定化后再刷新 project stage assessment。当前仍 `fresh_refresh_observed=false`、`quote_source=fallback_quotes_only`、`13` 条 post-refresh watch rows 等待，清单只定义 refresh 后怎么验收，不触发 M12.37 once-mode、不接 broker、不改参数、不改 registry/account specs/broker readiness，并已接入 M14 project stage assessment。
