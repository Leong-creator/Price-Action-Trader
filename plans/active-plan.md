...
- 下一阶段主线是继续用 `scripts/run_m12_37_intraday_auto_loop.py --session` 在美股开盘相关时段自动预热并持续刷新看板和 Codex 摘要；该入口已自动运行 M13 并在盘后运行 M14 固化每日 ledger/challenge/gate。M12.47 守护器必须使用配置化美股休市日历，周一假日不得被当作下一交易日拉起 M12.37。M13 最新盘中样本 `2026-05-20` 已显示所有 required 策略/模块都有有效账本状态且 `blocked_strategy_ids=[]`；M14.1 已修复 fallback close、stop/target 成交价、以及“旧持仓不在当天信号行就不更新报价”的全量持仓估值缺口。当前 M14 gate 仍未批准任何策略进入内部模拟成交，盘后固化有效 fully-ready challenge 为 `7/10` 天；`2026-05-20` 盘中已写 M13 账本，需等盘后 M14 固化后才计入第 `8/10` 天。下一步必须继续累计到 `10/10`，并继续同时记录 `PA004 baseline / PA004-MBF / PA004-MBF-QC` 三条独立账户。`M10-PA-002/007/012/013/M12-FTD-001` 当前已进入 `modify_candidate`，但 baseline 不静默覆盖，后续只能通过新变体 A/B 验证。
