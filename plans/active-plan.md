...
- M14 rescue zero-signal diagnostics 已落地：`scripts/run_m14_rescue_zero_signal_diagnostics.py` 当前把 `8` 条零信号策略拆到 `9` 个 runtime 粒度；结果显示 `7` 个 runtime 主要受当前 recompute/audit 快照的非 fresh quote gate 影响，应等下一次 M12.47 fresh quote 后复核，`1` 个 runtime 是 `M10-PA-012` 的 reward/R 过滤过严，适合 shadow-only 目标空间归一化，`1` 个 runtime 是 `M10-PA-013` 的 1d parent source 缺失，需修 timeframe/source mapping。该诊断已进入 M14 goal readiness，用于决定先等 fresh 数据、做参数影子测试还是修 detector 映射，不改变 broker/live 边界。
