...
- M14/M15 当前进行中：M14 已从策略级 `promote / modify / reject / continue_testing` 旧口径改成 `runtime_id` 级动作矩阵，`strategy_id` 只作为父策略分组。当前动作只允许 `advance_internal_sim`、`risk_limited_advance`、`repair_now`、`pause_runtime`、`paper_candidate`；高回撤和低胜率只产生风险提醒与仓位折扣，不再自动否掉盈利 runtime。M12/M14 活跃输出不再展示或引用旧版 `historical_net_profit` / 历史净利润 / 历史收益 / 历史盈利因子；未来历史表现必须基于 M13 account operation ledger 重新 mark-to-market。M15 已新增 Longbridge paper preflight scaffold，但当前因 `m12_quote_source=fallback_quotes_only` 阻断 paper candidate，且仍禁止凭证注入、broker 连接、订单提交、live token 和真实资金。
