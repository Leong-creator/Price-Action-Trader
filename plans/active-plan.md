...
- M14/M15 当前进行中：M14 已从策略级 `promote / modify / reject / continue_testing` 旧口径改成 `runtime_id` 级动作矩阵，`strategy_id` 只作为父策略分组。当前动作只允许 `advance_internal_sim`、`risk_limited_advance`、`repair_now`、`pause_runtime`、`auxiliary_module`、`paper_candidate`；高回撤和低胜率只产生风险提醒与仓位折扣，不再自动否掉盈利 runtime。M15 已把周一前修复队列固化为 `9` 个立即修复运行单元，逐条写明修复动作、验收条件、仓位倍率和长桥模拟账户阻断状态；其中仓位倍率、收益风险比、止损距离、假突破确认、图形上下文和连续亏损暂停已接入 M12 账户化运行链路，不再只是报告计划。M12/M14 活跃输出不再展示或引用旧版 `historical_net_profit` / 历史净利润 / 历史收益 / 历史盈利因子；未来历史表现必须基于 M13 account operation ledger 重新 mark-to-market。M15 已新增 Longbridge paper preflight、OAuth 连接审计、全策略订单预演、快速新信号队列和模拟账户提交器；其中提交器默认消费快速新信号队列，每笔订单只做快速风控，不再等待完整 M13/M14 重算。快速队列已支持同标的同方向合并、多策略共振加权、旧快照强阻断和盘前异动保护；共振只提高整股数量，最高倍率 `1.75`，仍受 `6000 USD` 资金模型、单标的/总敞口和单笔风险上限约束。完整本地模拟、M13/M14 评分和全策略预演保留为后台审计与对账；长桥模拟账户状态只来自长桥模拟账户自身，不迁移旧本地模拟仓位。当前仍只限模拟账户，live token、真实资金、碎股、融券做空和期权保持禁用。
