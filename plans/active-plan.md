...
- M14 project stage assessment 已落地：`scripts/run_m14_project_stage_assessment.py` 当前把 goal readiness、internal-sim next-session plan、rescue plan 与 rescue optimization backlog 合成单一项目阶段/策略路线图；最新结论为项目处于 `M14 stable strategy testing + M14.2 broker readiness dry-run scaffold`，10 日 challenge `10/10` 已满足，`M10-PA-004 / M10-PA-005 / M10-PA-008` 继续内部模拟，`8` 条救援路线继续收集 A/B 证据，`1` 条并行 A/B，`1` 条先重建 detector 后再判断，`7` 条仅保持 shadow/plugin/research 覆盖；目标仍未完成，因为 rescue promotion 仍为 `0`、`2` 条 rescue runtime 还缺首个 M13 ledger，且 broker/live/real order/paper approval 继续全禁用。
