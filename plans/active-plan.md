...
- 下一阶段主线是继续用 `scripts/run_m12_37_intraday_auto_loop.py` 在美股开盘时段持续刷新看板和 Codex 摘要，累计每日看板到 `10` 个真实交易日；同时继续扩充新闻/财报事件样本和第一批 `50` 只长历史 `5m`，长历史 `5m` 仍用于后续完整日内回测，不阻塞当前盘中只读看板。
