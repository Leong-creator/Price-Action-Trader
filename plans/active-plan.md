...
- M14 rescue runtime 覆盖审计已落地：`scripts/run_m14_rescue_runtime_coverage.py` 当前证明 M13 registry 中 `10/10` 条 rescue 策略、`11` 个 M12 account specs 输入流均已接线，M14 rescue plan 中 `10/10` 个 rescue/rebuild 动作已有 runtime 覆盖；该审计只证明输入覆盖，不代表策略通过或准入，所有 rescue runtime 仍必须继续累计 10 个交易日 A/B ledger 后再判断 `promote / modify / reject`。
