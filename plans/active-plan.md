...
- M14/M15 当前进行中：M14 已从策略级 `promote / modify / reject / continue_testing` 旧口径改成 `runtime_id` 级动作矩阵，`strategy_id` 只作为父策略分组。当前动作只允许 `advance_internal_sim`、`risk_limited_advance`、`repair_now`、`pause_runtime`、`auxiliary_module`、`paper_candidate`；高回撤和低胜率只产生风险提醒与仓位折扣，不再自动否掉盈利 runtime。M15 已把周一前修复队列固化为 `9` 个立即修复运行单元，逐条写明修复动作、验收条件、仓位倍率和长桥模拟账户阻断状态；其中仓位倍率、收益风险比、止损距离、假突破确认、图形上下文和连续亏损暂停已接入 M12 账户化运行链路，不再只是报告计划。M12/M14 活跃输出不再展示或引用旧版历史利润类字段；未来历史表现必须基于 M13 account operation ledger 重新 mark-to-market。M15 已新增 Longbridge paper preflight、OAuth 连接审计、全策略订单预演、快速新信号队列、旧模拟账户提交器、新的长桥实时行情事件采集器、实时信号路由器、实时账户状态读取器、实时持仓退出管理、实时执行链路和实时会话守护器；行情事件采集器第一版只调用 Longbridge CLI 只读 `kline` 命令生成 `m15_realtime_market_events.jsonl`，不读取账户、订单或本地模拟，并用热名单优先和游标分批降低轮询延迟；实时信号路由器只消费实时行情/新 K 线事件，输出实时信号事件，第一版支持内嵌策略意图、`M10-PA-004-long-1d` 最小检测器和 `price_action_realtime_v1` 通用实时价格行为检测器，已覆盖白名单内 `M10-PA-001/002/005/008/012/013/FTD001` 的日线或 5 分钟运行单元，同标的同方向多运行单元共振时只生成一条主信号；实时账户状态读取器只读取长桥模拟账户自身现金、持仓和挂单；实时持仓退出管理只用长桥持仓、长桥行情和长桥执行记录里的止损/目标价生成平仓信号；实时执行链路只消费实时信号事件和长桥模拟账户自身状态，不再读取本地模拟交易账本、不再用本地平仓阻断长桥开仓、不再把 M13/M14 完整重算作为每笔下单前置，并检查已有持仓/挂单、现金和敞口；实时会话守护器只在常规交易时段按顺序串联采集、路由、账户状态、持仓退出和执行，非交易窗口只写等待状态。快速队列和全策略预演降级为本地审计/历史兼容产物。长桥实时链路以 `1` 秒内本地处理为优化目标，第一版 `5` 秒内可接受，超过 `5` 秒必须重新校验当前价格、风控和扣费后利润后再决定。当前仍只限长桥模拟账户，资金模型 `10000 USD`，live token、真实资金、碎股、融券做空和期权保持禁用。
