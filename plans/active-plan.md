...
- 2026-08-07：完成重复占仓与账户对账稳定性收口。实时执行器现在从请求已发起开始持续保留资金池敞口，覆盖“券商快照尚未出现订单”“订单已成交但成交归因尚未刷新”“部分成交后终态”三个交接窗口；只有零成交终态或精确归因确认已平仓才释放。日线信号新增按测试编号、运行单元、标的、方向和纽约交易日固定的结构实例编号，同一日线结构不能因后续五分钟轮询事件变化而重复开仓。历史虚拟批次通过真实长桥订单证据完成 `33` 条不计策略成绩的账户对账调整，当前 `20` 个长桥实际持仓与虚拟归因逐标的数量差异为 `0`、归因异常为 `0`。合同版 SDK 三件套是唯一生产入口；正式测试标记激活后统一写 `blocks_new_entries=false`，超限资金池仍由各自 `6000 USD` 上限单独冻结。完整聚焦回归 `347` 项通过。
