...
- M12.32 已优化分钟级中文只读模拟看板：首屏改为共享模拟账户视角，展示当前模拟权益、今日机会、盘中模拟盈亏、模拟收益率、浮盈机会占比、最大回撤参考和可用策略数；第二层按单策略独立展示历史收益、胜率、最大回撤、盈利因子、今日模拟盈亏；第三层保留今日机会明细和单策略下钻数据。
...
- M14 当前进行中：已从 M13 前置基线切出 `codex/m14-strategy-challenge-paper-gate`；当前目标是在 M13 每日账本之上建立 append-only `10` 个纽约交易日 challenge、策略 `promote / modify / reject / continue_testing` 决策账本、内部模拟准入 gate 和显眼的数据刷新告警。M12.37 已通过 `post_run_strategy_ledgers` 自动接入 M13/M14：每次刷新后更新 M13 当日账本，M14 默认只在盘后固化 challenge/gate。`M10-PA-004` 不按强势行情追涨改写 baseline，已新增 `M10-PA-004-MBF` 强势突破跟进实验变体作为独立 A/B 账户；由于原 MBF 早期触发 `-2R` 改进条件，现新增 `M10-PA-004-MBF-QC` 质量确认并行改进变体，原 MBF 不冻结，继续跑满 10 个交易日后再最终判断。M14.1 已开始把 M12/M13/M14 看板改成券商式交易终端首屏，新增 `broker_terminal_view`、默认自选股分组（核心 ETF、美股七姐妹、AI/芯片、存储/闪存热点、策略活跃标的）和新闻/催化面板；这些模块只服务观察与复盘，不作为交易触发或准入批准。M14.1 同步修复所有策略共享的账户 runtime 平仓语义：fallback/cached/no-refresh quote 不得触发平仓，止损/止盈成交价按 stop/target 结算，并用修复审计文件记录旧账本纠偏。M14.1 还修复了看板数据刷新链路：盘中 K 线补数按 5 分钟 bucket 触发、预算提升到 `100`、当前 `5m` 同日缓存可强制刷新，first50 真实样本已恢复 `50/50` 且 freshness 告警清空；M14 challenge 现在只用 fully-ready 交易日计入有效 `10` 日挑战，降级日保留审计但不污染后续 gate。
