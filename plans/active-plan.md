...
- 2026-08-05：新基线固定为 `m15-sdk-contract-v1-20260806` / `m15-sdk-contract-v1-short-20260806`。上线前必须由只读切换预检确认模拟账户、合同、独立分仓和配置一致，然后进入 `pending_flatten`；只有长桥实际持仓、挂单、待确认订单全部为零才能自动激活。当前旧运行配置继续工作，未通过完整回归前不得热切换。
