...
- 2026-07-15 M15 SDK 全量切换进入两日只读验收：M15 自动交易、账户快照和后台看护均已改走 Longbridge SDK，自动路径禁止调用 CLI，也不读取 M12/M13/M14 或本地模拟账本。实测 SDK `QuoteContext/TradeContext/PortfolioContext`、模拟账户、`147/147` 订阅、`147×60` 日线缓存和账户快照均正常。五分钟 K 线在 `150ms` 内合并后才触发一次路由/风控，日线与五分钟输入独立保留；日线获取独立于订阅进程并有超时回收，不能再拖慢实时推送。订单提交仍为模拟账户专用，并由“两个完整常规交易日只读验收”门禁控制：每个通过日必须具备完整订阅、完整日线、健康账户快照和真实行情事件。当前 `0/2`，未满足前禁止自动创建模拟订单；到 `2/2` 后才允许当时新产生且通过现有策略/风控的信号提交。旧 CLI 守护器、旧快速队列和本地模拟开平仓均不再是 M15 下单来源。
