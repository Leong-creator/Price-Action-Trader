...
- M14 objective completion audit 已补强：`scripts/run_m14_objective_completion_audit.py` 当前把用户目标拆成 `12` 项机器可读需求审计，并直接读取 parameter shadow spec 与 strategy decision ladder 作为弱策略救援/调参路径证据；其中 `4` 项 proven（项目阶段、10 日 challenge、approved internal sim、外部参考映射）、`3` 项 guardrail（内部模拟口径、broker/live 禁用、手动 M12.37 once-mode 禁用）、`2` 项 in_progress（弱策略救援、参数实验路径）、`3` 项 blocked（rescue promotion 证据、fresh refresh 前置、整体目标完成）。当前 `objective_complete=false`，原因是 rescue promotion 仍为 `0`、fresh refresh 尚未出现、parameter activation shadow-review candidate 仍为 `0`；审计已确认 `14` 个参数影子变体、`10` 条 rescue-continuation ladder rows、`final_discard_allowed_count=0`，并已接入 M14 project stage assessment，继续保持只读/模拟，不允许 broker/live、参数 mutation、registry/account-spec/broker readiness mutation 或手动 M12.37 once-mode。
