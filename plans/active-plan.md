...
- M14.2 已补上真实 artifact 生成器：`scripts/run_m14_2_broker_readiness_scaffold.py` 只读 M14 gate 与内部模拟执行账本，批量生成 `m14_2_broker_readiness/broker_readiness_plan.json` 和 `broker_readiness_audit.jsonl`。当前样本显示 `8` 条内部模拟 `risk_check` 中 `1` 条 broker dry-run ready、`7` 条被风控阻断；下一步策略救援应优先减少低质量信号和超敞口/超单笔风险触发，不得为了提高通过率放宽风控。
