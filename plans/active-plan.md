...
- M12.34-M12.39 当前已进入自动盘中测试与 Codex 观察员阶段：观察策略不再是空状态，已进入低准入每日测试 lane；看板已按 `1d / 1h / 15m / 5m` 分组；`M12-FTD-001` 已单独展示重点监控；自动运行器、Codex 观察员 inbox 与 Codex App 心跳自动化 `m12` 已落地，仍只做只读行情、模拟盈亏和中文摘要。
