...
- 2026-08-03 新增 `M10-PA-002-5m-repaired-v1` 长桥 SDK 模拟账户独立单策略仓，使长桥允许交易运行单元由 `18` 增至 `19`，并与现行 `M10-PA-002-5m` 并行而不替换：两仓各自按 `10000 USD` 虚拟资金、`6000 USD` 总敞口、`1500 USD` 单标的、`20 USD` 单笔风险和 `4000 USD` 现金保留运行，倍率均为 `1.0`。修复版只消费长桥 SDK 实时五分钟行情，要求突破后最新一根继续站稳、目标至少 `1.3R`、风险距离不超过 `2.5%`；每两笔长桥实际完整亏损后只跳过下一个合格信号，持仓最迟在下一纽约交易日退出。它不注册为本地模拟运行单元，也不读取本地账本。
