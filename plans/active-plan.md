...
- M14 rescue A/B evidence tracker 已落地：`scripts/run_m14_rescue_ab_evidence_tracker.py` 当前把 rescue coverage、M14 gate、M13 scorecard 与 M13 signal/account ledger 合成取证门禁；最新结果显示 rescue runtime coverage 完整，但 rescue 变体自己的 M13 ledger evidence 为 `0/9`，9 条 rescue 变体均仍缺 10 日 A/B 证据，父策略历史不得折算为 rescue 变体通过证据，promotion allowed 固定为 `0`。
